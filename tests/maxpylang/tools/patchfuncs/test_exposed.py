...
def test_set_position_outputs_and_reorder_renumbers_objects(capsys):
...
def test_replace_handles_missing_and_existing_objects_and_preserves_connections(capsys):
    patch = MaxPatch(verbose=False)
...
def test_inspect_returns_none_for_all_and_specific_objects(capsys):
    patch = MaxPatch(verbose=False)
    obj = patch.place("toggle", verbose=False)[0]

    assert patch.inspect() is None
    assert patch.inspect("all") is None
    assert patch.inspect(obj._dict["box"]["id"]) is None
